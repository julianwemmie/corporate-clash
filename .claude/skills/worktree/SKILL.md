...
Create a git worktree with dependencies installed, so the worktree is immediately ready to run the dev server.
...
2. Runs `bun install` in the new worktree
3. Prints a summary with the path to `cd` into
